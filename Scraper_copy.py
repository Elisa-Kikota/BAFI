...
file_path = os.getcwd() + '\\Records_Taken\\'
...
matches_data = pd.DataFrame(columns=["Country","Club_name", "Date", "Venue", "GF", "GA", "Opponent"])

#Creating last match saved data frame
last_match_data = pd.DataFrame(columns=["Country","Club_name", "Date", "Venue", "GF", "GA", "Opponent"])
    ...
    if os.path.exists(file_path_for_all_data):
        ...
        for country in allCountriesPage.find("table",{"id":"countries"}).find("tbody").find_all("tr")[start_index:]:
            ...
            if response_Country.status_code == 200:
                ...
                new_match = None  # Define new_match outside the loo
                                ...
                                for Match in ClubPage.find("table",{"id":"matchlogs_for"}).find("tbody").find_all("tr"):
                                    ...
                                    new_match = [
                                        {"Country"  : country_name,
                                         ...
                                         "Venue"    : Match.find("td",{"data-stat":"venue"}).text,
